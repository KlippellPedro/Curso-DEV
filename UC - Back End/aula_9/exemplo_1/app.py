...
def calcular_folha(valor_hora, horas_trabalhadas,desconto_percentual):
    ...
    # retona todos os valores calculados
...
def calcular():
    try:
        # Recebe valor da hora e converte para float
        ...
        # Cahama a função de calculo
        salario_bruto,valor_desconto,salario_liquido=calcular_folha(
            valor_hora,horas_trabalhadas,desconto
        )
        # Envia os dados para a página de resultado
        return render_template("resultado.html", valor_hora=valor_hora,horas_trabalhadas=horas_trabalhadas,desconto=desconto,salario_bruto=salario_bruto,valor_desconto=valor_desconto,salario_liquido=salario_liquido) 
    except ValueError:
        # Caso ocorra erro de conversão
        erro= "Erro ao processar os dados. Digite valores válidos"
        return render_template("index.html", erro=erro)
