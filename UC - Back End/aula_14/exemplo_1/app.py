# pip install reportlab (biblioteca para PDF)
...
def gerar_pdf(funcionario):
    ...
    elementos.append(Paragraph(f"Salário bruto R$ {funcionario.salario_bruto:.2f}", estilos["Normal"]))
    elementos.append(Paragraph(f"INSS R$ {funcionario.inss:.2f}", estilos["Normal"]))
    elementos.append(Paragraph(f"IRRF R$ {funcionario.irrf:.2f}", estilos["Normal"]))
    elementos.append(Paragraph(f"Desconto VA R$ {funcionario.desconto_va:.2f}", estilos["Normal"]))
    elementos.append(Paragraph(f"Desconto VT R$ {funcionario.desconto_vt:.2f}", estilos["Normal"]))
    elementos.append(Paragraph(f"Salário líquido R$ {funcionario.salario_liquido:.2f}", estilos["Normal"]))
